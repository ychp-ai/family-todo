...
const initialTasks = [
  ...
  { id: 'coat', title: '取回干洗的外套', family: 'personal', person: 'self', date: 'today', time: '', done: false, group: 'self', manage: true, canComplete: true },
  ...
  { id: 'read', title: '阅读 20 分钟', family: 'mine', person: 'child', date: 'today', time: '16:00', done: true, group: 'help', repeat: true, manage: true, canComplete: true, operator: '奶奶', completedAt: '16:24', visible: ['爸爸', '奶奶'], helpers: ['爸爸', '奶奶'], note: '读完后，可以和家人说说今天最喜欢的故事。' },
  { id: 'bag', title: '整理明天的书包', family: 'mine', person: 'child', date: 'today', time: '19:30', done: false, group: 'help', repeat: true, manage: true, canComplete: true, visible: ['爸爸', '奶奶'], helpers: ['爸爸', '奶奶'] },
  ...
  { id: 'toys', title: '收好玩具', family: 'mine', person: 'child', date: 'today', time: '', done: true, group: 'help', manage: true, canComplete: true, operator: '奶奶', completedAt: '15:40', visible: ['爸爸', '奶奶'], helpers: ['爸爸', '奶奶'] },
  { id: 'photos', title: '整理旅行照片', family: 'personal', person: 'self', date: '2026-09-10', time: '', done: false, group: 'self', manage: true, canComplete: true }
];
...
const isDue = (task) => task.reminder && !task.done && !task.skipped && task.date === 'today' && task.time <= '18:30';
...
function showQuickAdd() {
  ...
  openSheet('记一件事', `<form id="quick-form"><input class="title-input" id="task-title" maxlength="80" placeholder="想记下什么小事？" aria-label="事项标题" required autocomplete="off"><span class="field-label">哪天做</span><div class="form-pills">${[['today', '今天'], ['tomorrow', '明天'], ['unscheduled', '未安排']].map(([date, label]) => `<button type="button" data-new-date="${date}" class="${chosenDate === date ? 'active' : ''}" aria-pressed="${chosenDate === date}">${label}</button>`).join('')}</div><div id="basic-privacy" class="privacy-note">${icon('lock')}<span>个人事项 · 仅自己可见<br>不设置时刻和提醒，也可以保存。</span></div><button type="button" id="expand-settings" class="sheet-secondary">家庭、重复与共享${icon('chevron')}</button><div id="advanced-fields" hidden><label class="field-label" for="new-family">所属家庭</label><select id="new-family" class="form-select"><option value="personal">个人 · 暂不归属家庭</option><option value="mine">我的小家</option><option value="parents">爸妈家</option></select><label class="field-label" for="new-person">谁来做</label><select id="new-person" class="form-select"><option value="self">自己</option></select><div class="two-fields"><div><label class="field-label" for="new-repeat">重复</label><select id="new-repeat" class="form-select"><option value="once">不重复</option><option value="daily">每天</option></select></div><div><label class="field-label" for="new-time">时刻（可选）</label><input id="new-time" class="form-input" type="time"></div></div><p class="form-helper" id="new-privacy">仅自己可见，未开启提醒。</p><div id="new-viewers"></div></div><p id="quick-error" class="form-error" role="alert"></p><p id="quick-success" class="sheet-success" role="status"></p><div class="sheet-actions"><button type="button" class="outline-button" id="save-next">保存并继续记</button><button class="primary-button" type="submit">保存事项</button></div></form>`, () => {
    ...
    function updatePrivacy() {
      ...
      $('#new-privacy').textContent = person === 'child' ? '小宝由家人代记。事项归属妈妈（家庭拥有人），由你管理；未默认向全家公开。' : person === 'self' ? '默认仅自己可见。增加可见人不会同时开启提醒或代记权限。' : `为${people[person].name}安排时，${people[person].name}需要可见；其他家人不默认加入。`;
    ...
    function save(continueAdding) {
      ...
      tasks.push({ id: `new-${crypto.randomUUID()}`, title, family, person, date: chosenDate, time, done: false, group: person === 'self' ? 'self' : 'help', manage: true, canComplete: true, repeat, visible: expanded ? $$('#new-viewers input:checked').map((input) => input.value) : [] });
