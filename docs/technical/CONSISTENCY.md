...
条件读取复用 `query_sessions` 的不可变 `list-validation/v1` 元数据，会话绑定 actor、查询指纹、scope revision、家庭/version/membership 向量、固定 asOf、nextInvalidationAt 与至多15分钟的 expiresAt，不保存列表结果。只有完整扫描、全部 scope 成功且末尾权限/版本栅栏通过才生成；历史 cursor 缺少边界证明时不生成。命中只读会话并在有界事务内核验 actor/scope/family/slot/member，返回前再次检查时间；不扫描 roster、事项、片段、控制、次数、偏好或回执，不写入或续期会话。

事项/提醒扫描在过滤前累计未来 eligibilityBoundary 和 scheduledAt，跨分页及历史窗口保留最早边界，并加入下一个上海午夜。因此空提醒也能在到时失效，canRecord 与日期型次数不会跨午夜复用。扫描结束已越过边界时不给验证器；family.list 只使用自然到期。缓存失效回退既有批量全量路径，保留返回前版本栅栏。真实云端成本与双账号撤权验证仍须单独验收。
