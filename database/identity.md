...
以下集合已通过 CLI 显式创建并设为 ADMINONLY，开发者工具直接读写拒绝。创建记录见 [个人待办迁移](personal.md)。
...
创建时在一个事务内按键读取映射，存在则读取关联用户；不存在则确认随机 UUID 无冲突，并一次提交用户、零计数容量文档和映射。事务最多 6 次文档操作；明确冲突由适配器在回滚后最多退避重试 3 次，总执行上限 4 次。网络响应丢失后的再次调用使用相同映射键，不会重复建用户。该唯一键是 identity.ensure 的幂等依据，个人业务写入另用 idempotency_receipts 保存 requestId 回执。
