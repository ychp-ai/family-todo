...
查询会话复用 query_sessions，actor、查询指纹、个人 revision、所有家庭 version、asOf 及 15 分钟过期时间绑定；存储大小上限 128 KiB。每个请求最多 180 工作单位并保留 4 秒事务预算，历史范围不截断：每个窗口不超过 31 天，积压/提醒从最近窗口逐窗向任务最早日期遍历。窗口内按 localDate、time（日期型为99:99）、taskId、occurrenceId 排序；只保存 limit+1 最小候选，完整扫描该窗口后才发布一页。未完成扫描时允许返回空页及继续游标；summary 只累计已经发布的去重次数，全部窗口及范围完成且无失败家庭时才返回最终值。暂停/停止历史仍参与投影，控制逐次校验，稀疏状态按 ID 覆盖。不存在任意 90 天或 365 天历史上限。
