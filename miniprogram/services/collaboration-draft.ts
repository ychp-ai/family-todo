...
export function permissionRows(roster:Roster,subject:Subject,previous:ParticipantDTO[] = [], existingTask = false): PermissionRow[] {
  ...
  return roster.members.map(member=>{
    ...
    const requiredViewer=legacyManagement ? old?.requiredViewer===true : isCreatorManager===true || (subject.kind === "member" && member.id===subject.membershipId) || (subject.kind === "virtual" && member.id===roster.family.ownerMembershipId);
