...
import { collect, authorizedItems } from "./personal-lists";
...
describe("家庭服务与分页",()=>{
  ...
  it("最终作用域失败时丢弃早先分页条目，不泄漏已失权家庭内容",()=>{const items=[{familyId:null},{familyId:"family-a"},{familyId:"family-b"}];expect(authorizedItems(items,[{familyId:null,status:"ok"},{familyId:"family-a",status:"failed"},{familyId:"family-b",status:"ok"}],item=>item.familyId)).toEqual([items[0],items[2]]);});
