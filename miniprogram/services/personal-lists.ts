...
export async function listReminders(includeDismissed = false, active?: ActiveCheck) {
  const result = await collect<ReminderDTO, AggregatePage<ReminderDTO>>(
    cursor => personalApi.read("reminder.list", { includeDismissed, limit: 50, ...(cursor ? { cursor } : {}) }),
    active,
  );
  return {
    ...result,
...
export function listHistory(taskId: string, active?: ActiveCheck) {
  return collect<TaskEventDTO, Page<TaskEventDTO>>(
    cursor => personalApi.read("task.history", { taskId, limit: 50, ...(cursor ? { cursor } : {}) }),
    active,
  );
}
