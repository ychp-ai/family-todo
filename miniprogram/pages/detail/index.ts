...
Page({
  onComponentAction: componentActions(["deleteTask", "edit", "recycle", "share", "toggleReminder"]),
  ...
  selectedOccurrence:null as OccurrenceRef|null,occurrencesEpoch:0,visible:false,alive:true,epoch:0,pendingWrite:null as null | {key:string;run:()=>Promise<unknown>;done:()=>void},
  onLoad(query: Record<string,string | undefined>) {patchData(this, {...navigationMetrics(),id:query.id ?? ""});if(query.occurrence){try{const ref:unknown=JSON.parse(decodeURIComponent(query.occurrence));if(!isOccurrenceRef(ref)||ref.taskId!==this.data.id)throw new Error("次数参数无效。");this.selectedOccurrence=ref;}catch{patchData(this, {status:"error",error:"次数参数无效，请返回列表重新打开。"});this.alive=false;}}},
  onShow() {if(!this.alive)return;this.visible=true;void this.refresh();},
  onHide() {this.stop();},onUnload() {this.alive=false;this.stop();},
  stop(){this.visible=false;this.epoch++;this.occurrencesEpoch++;},
  back,
  async refresh(){
    if(!this.visible||this.pendingWrite||this.data.writing)return;const epoch=++this.epoch;this.occurrencesEpoch++;patchData(this, {occurrencesLoading:false,occurrencesError:""});if(!this.data.task)patchData(this, {status:"loading"});
    try{await readySession();const [result,history]=await Promise.all([personalApi.read("task.get",{id:this.data.id,...(this.selectedOccurrence?{occurrence:this.selectedOccurrence}:{})}),listHistory(this.data.id,()=>this.visible&&epoch===this.epoch)]);if(!this.visible||epoch!==this.epoch)return;
      ...
      patchData(this, {status:"ready",error:"",historyDate:this.data.historyDate||o?.localDate||dateAt(serverNow),seriesLabel:result.task.schedule.kind==="once"?"一次性事项":`${result.task.schedule.kind==="daily"?"每天":"每周"} · ${{active:"进行中",paused:"已暂停",stopped:"已停止",deleted:"已删除"}[result.task.lifecycle]}`,hasTimes:result.task.schedule.kind==="once"?!!result.task.schedule.time:result.task.schedule.times.length>0,task:result.task,occurrence:o,viewerNames:result.task.familyId?result.task.participants.filter(p=>p.canView).map(p=>p.name).join("、"):"仅自己可见",helperNames:result.task.familyId?result.task.participants.filter(p=>p.canHelp).map(p=>p.name).join("、")||"未指定":"仅自己",completionLabel:o?.status==="completed"?"已完成":o?.status==="skipped"?"已跳过":"待完成",recordedLabel:instantCaption(o?.recordedAt??null),actualLabel:instantCaption(o?.actualCompletedAt??null),events:history.items.map(e=>({...e,label:labels[e.kind]??e.kind,recordedLabel:instantCaption(e.recordedAt),actualLabel:instantCaption(e.actualCompletedAt)})),...(this.data.sheet==="record"?{}:{recordDate:local.slice(0,10),recordTime:local.slice(11,16)})});if(result.task.schedule.kind!=="once")void this.loadOccurrences();
    }catch(error){if(!this.visible||epoch!==this.epoch)return;if(isAccessDenied(error))this.clearAccess();patchData(this, {status:"error",error:errorMessage(error),task:null,occurrence:null,events:[],occurrences:[],sheet:"",viewerNames:"",helperNames:""});}
  },
  retry(){void this.refresh();},
  settings(){patchData(this, {sheet:"settings",writeError:""});},
  closeSheet(){if(this.data.writing||this.data.uncertain){wx.showToast({title:"请先重试确认操作结果",icon:"none"});return;}patchData(this, {sheet:""});},noop(){},
  edit(){if(this.data.uncertain||!this.data.task?.capabilities.canEdit)return;patchData(this, {sheet:""});wx.navigateTo({url:`/pages/editor/index?id=${encodeURIComponent(this.data.id)}`});},
  share(){if(this.data.uncertain||!this.data.task?.capabilities.canShare)return;patchData(this, {sheet:""});wx.navigateTo({url:`/pages/editor/index?id=${encodeURIComponent(this.data.id)}&share=1`});},
  ...
  async runWrite(key:string,run:()=>Promise<unknown>,done:()=>void=()=>{}) {
    ...
    this.pendingWrite??={key,run,done};this.epoch++;this.occurrencesEpoch++;patchData(this, {occurrencesLoading:false,occurrencesError:""});patchData(this, {writing:true,writeError:""});
    ...
    catch(error){const uncertain=error instanceof PersonalApiError&&error.retryable;if(!uncertain)this.pendingWrite=null;if(this.alive){const denied=isAccessDenied(error);if(denied)this.clearAccess();patchData(this, {writeError:errorMessage(error),uncertain,...(denied?{task:null,occurrence:null,events:[],occurrences:[],viewerNames:"",helperNames:"",sheet:"",status:"error",error:errorMessage(error)}:{})});wx.showToast({title:errorMessage(error),icon:"none"});}}
    finally{if(this.alive)patchData(this, {writing:false});if(!this.pendingWrite&&this.visible)void this.refresh();}
  },
  ...
  openRecord(){if(this.data.uncertain||!this.data.occurrence?.canRecord)return;patchData(this, {sheet:"record",writeError:""});},
  recordDateChange(e:WechatMiniprogram.PickerChange){if(typeof e.detail.value==="string")patchData(this, {recordDate:e.detail.value});},
  recordTimeChange(e:WechatMiniprogram.PickerChange){if(typeof e.detail.value==="string")patchData(this, {recordTime:e.detail.value});},
  recordNoteInput(e:WechatMiniprogram.Input){patchData(this, {recordNote:e.detail.value});},
  async record(e:WechatMiniprogram.TouchEvent){
    ...
    const actual=new Date(`${this.data.recordDate}T${this.data.recordTime}:00+08:00`);if(status==="completed"&&!Number.isFinite(actual.getTime())){patchData(this, {writeError:"请选择实际完成时间。"});return;}
    ...
    await this.runWrite("record",()=>personalApi.write("occurrence.record",payload),()=>patchData(this, {sheet:"",recordNote:""}));
  },
  clearAccess(){this.epoch++;this.occurrencesEpoch++;patchData(this, {task:null,occurrence:null,events:[],occurrences:[],occurrencesLoading:false,occurrencesError:"",viewerNames:"",helperNames:"",sheet:"",recordNote:"",status:"error"});},
  async loadOccurrences(){if(!this.visible||!this.data.task)return;const epoch=++this.occurrencesEpoch;const mainEpoch=this.epoch;const date=this.data.historyDate;patchData(this, {occurrencesLoading:!this.data.occurrences.length,occurrencesError:""});try{const result=await collect<OccurrenceDTO,ContractPage<OccurrenceDTO>>(cursor=>personalApi.read("occurrence.list",{taskId:this.data.id,dateFrom:date,dateTo:date,limit:50,...(cursor?{cursor}:{})}),()=>this.visible&&epoch===this.occurrencesEpoch&&mainEpoch===this.epoch);if(this.visible&&epoch===this.occurrencesEpoch&&mainEpoch===this.epoch)patchData(this, {occurrences:result.items,occurrencesLoading:false});}catch(error){if(this.visible&&epoch===this.occurrencesEpoch&&mainEpoch===this.epoch){const denied=isAccessDenied(error);if(denied)this.clearAccess();patchData(this, {occurrencesLoading:false,occurrences:[],occurrencesError:errorMessage(error),...(denied?{task:null,occurrence:null,events:[],occurrences:[],viewerNames:"",helperNames:"",sheet:"",status:"error",error:errorMessage(error)}:{})});}}},
  historyDateChange(e:WechatMiniprogram.PickerChange){if(typeof e.detail.value!=="string")return;patchData(this, {historyDate:e.detail.value,occurrences:[]});void this.loadOccurrences();},
