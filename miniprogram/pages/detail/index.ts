...
Page({
  ...
  back,
  async refresh(){
    if(!this.visible||this.pendingWrite||this.data.writing)return;const epoch=++this.epoch;this.occurrencesEpoch++;patchData(this, {occurrencesLoading:false,occurrencesError:""});this.lastRefresh=Date.now();if(this.timer)clearTimeout(this.timer);if(!this.data.task)patchData(this, {status:"loading"});
    ...
    }catch(error){if(!this.visible||epoch!==this.epoch)return;if(isAccessDenied(error))this.clearAccess();patchData(this, {status:"error",error:errorMessage(error),task:null,occurrence:null,events:[],occurrences:[],sheet:"",viewerNames:"",helperNames:""});const delay=[5000,15000,30000][this.retries++];if(delay&&(!(error instanceof PersonalApiError)||error.retryable||error.code==="CURSOR_EXPIRED"))this.schedule(delay);}
  },
  ...
  async runWrite(key:string,run:()=>Promise<unknown>,done:()=>void=()=>{}) {
    ...
    this.pendingWrite??={key,run,done};this.epoch++;this.occurrencesEpoch++;patchData(this, {occurrencesLoading:false,occurrencesError:""});if(this.timer)clearTimeout(this.timer);patchData(this, {writing:true,writeError:""});
  ...
  clearAccess(){this.epoch++;this.occurrencesEpoch++;if(this.timer)clearTimeout(this.timer);patchData(this, {task:null,occurrence:null,events:[],occurrences:[],occurrencesLoading:false,occurrencesError:"",viewerNames:"",helperNames:"",sheet:"",recordNote:"",status:"error"});},
