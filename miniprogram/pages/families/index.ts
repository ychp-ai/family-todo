...
Page({
  ...
  async load(){if(this.pending)return;const epoch=++this.epoch;patchData(this, {error:"",exit:null,transfer:null,previewLoading:false,sheet:"",...(!["ready","empty"].includes(this.data.status)?{status:"loading"}:{})});try{const user=await readySession();if(!this.visible||epoch!==this.epoch)return;this.profileUserId=user.id;const profile=readLocalProfile(user.id);const displayName=profile?.displayName??user.displayName;patchData(this, {displayName,avatarInitial:[...displayName][0]??"我",avatarPath:profile?.avatarPath??""});const result=await listFamilies();if(!this.visible||epoch!==this.epoch)return;const id=result.items.some(f=>f.id===this.data.id)?this.data.id:"";const progressDate=dateAt(result.last.asOf);const progressRead=id?Promise.allSettled([familyProgress(id,progressDate,()=>this.visible&&epoch===this.epoch)]):undefined;let detail=null;let virtualMembers:VirtualDTO[]=[];if(id){detail=await familyApi.read("family.get",{id});virtualMembers=detail.virtualMembers;if(detail.family.myMembershipId===detail.family.ownerMembershipId){virtualMembers=await listManagedVirtualMembers(id);}}if(!this.visible||epoch!==this.epoch)return;patchData(this, {status:result.items.length?"ready":"empty",families:result.items,id,family:detail?.family??null,members:(detail?.members??[]).map(member=>({...member,...(this.data.members.find(m=>m.id===member.id)?.progress?{progress:this.data.members.find(m=>m.id===member.id)?.progress}:{})})),virtualMembers:virtualMembers.filter(member=>member.status==="active").map(member=>({...member,...(this.data.virtualMembers.find(m=>m.id===member.id)?.progress?{progress:this.data.virtualMembers.find(m=>m.id===member.id)?.progress}:{})})),owner:detail?.family.myMembershipId===detail?.family.ownerMembershipId&&!!detail,ownerName:detail?.members.find(m=>m.role==="owner")?.name??"",targets:detail?.members.filter(m=>!m.isMe)??[],progressDate});if(id)void this.loadProgress(progressRead);}catch(error){if(this.alive&&epoch===this.epoch){if(isAccessDenied(error))this.clearCurrentFamily();patchData(this, {status:"error",error:errorMessage(error),family:null,members:[],virtualMembers:[],sheet:""});}}},
